...
bool RCGpuKang::Prepare(EcPoint _PntToSolve, int _Range, int _DP, EcJMP* _EcJumps1, EcJMP* _EcJumps2, EcJMP* _EcJumps3, EcJMP* _EcJumps4)
{
    ...
    EcJumps2 = _EcJumps2;
    EcJumps3 = _EcJumps3;
    EcJumps4 = _EcJumps4; // <-- ASSIGNMENT ADDED
    StopFlag = false;
    Failed = false;
    u64 total_mem = 0;
...
//allocate gpu mem
    ...
    // Allocate Jumps4 memory <-- ADDED
    total_mem += JMP_CNT * 96;
    err = cudaMalloc((void**)&Kparams.Jumps4, JMP_CNT * 96);
    if (err != cudaSuccess)
    {
        printf("GPU %d Allocate Jumps4 memory failed: %s\n", CudaIndex, cudaGetErrorString(err));
        return false;
    }
...
//jmp4 <-- ADDED
    ...
    for (int i = 0; i < JMP_CNT; i++)
    {
        memcpy(buf + i * 12, EcJumps4[i].p.x.data, 32);
        memcpy(buf + i * 12 + 4, EcJumps4[i].p.y.data, 32);
        memcpy(buf + i * 12 + 8, EcJumps4[i].dist.data, 32);
    }
    err = cudaMemcpy(Kparams.Jumps4, buf, JMP_CNT * 96, cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
        printf("GPU %d, cudaMemcpy Jumps4 failed: %s\n", CudaIndex, cudaGetErrorString(err));
        return false;
    }
    free(buf);
...
void RCGpuKang::GenerateRndDistances()
{
    ...
    {
        EcInt d;
        if (i < GRP_SIZE) // TAME kangs (1st quarter)
            d.RndBits(Range - 4); 
        else // WILD kangs (rest of the population)
        {
            d.RndBits(Range - 1);
...
bool RCGpuKang::Start()
{
    ...
    for (int i = 0; i < KangCnt; i++)
    {
        if (i < GRP_SIZE) // TAME (1st quarter) - offset 0
            memset(RndPnts[i].x, 0, 64);
        else if (i < 2 * GRP_SIZE) // WILD1 (2nd quarter) - offset PntA
            memcpy(RndPnts[i].x, buf_PntA, 64);
        else if (i < 3 * GRP_SIZE) // WILD2 (3rd quarter) - offset PntB
            memcpy(RndPnts[i].x, buf_PntB, 64);
        else // WILD3 (4th quarter) - offset PntB (reusing PntB for the new group)
            memcpy(RndPnts[i].x, buf_PntB, 64);
    }
