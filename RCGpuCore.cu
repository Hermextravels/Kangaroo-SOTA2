...
// Constant memory jump tables
__device__ __constant__ u64 jmp1_table[8 * JMP_CNT];
__device__ __constant__ u64 jmp2_table[8 * JMP_CNT];
__device__ __constant__ u64 jmp3_table[8 * JMP_CNT];
__device__ __constant__ u64 jmp4_table[8 * JMP_CNT];
...
struct KernelParams {
    u32 DP;
    u32 GroupCnt;
    u64* L2;      // Points x,y coordinates
    u64* L2s;     // Temporary storage
    u32* DPs_out; // Distinguished point flags
    u64* Jumps1;  // Main jump table
    u64* Jumps4;  // Emergency escape jumps
    u64* Jumps2;  // Second jump table
    u64* Jumps3;  // Third jump table
    u32* DPTable; // Distinguished point lookup table
    u32* L1S2;    // L1 state flags
};
...
#define EMERGENCY_ESCAPE     4   // New state for emergency escape jumps
#define JMP4_TRIGGER_MASK    0x0F00  // Mask to check for emergency escape conditions
#include "RCGpuUtils.h"


//jump tables for all kernels
__device__ __constant__ u64 jmp1_table[8 * JMP_CNT];
__device__ __constant__ u64 jmp2_table[8 * JMP_CNT];
__device__ __constant__ u64 jmp3_table[8 * JMP_CNT];
